...
class CustomCallback:
    ...
    args = ["version", "n_epochs", "keys_print", "freq_log", "freq_print"]
    ...
    def __init__(self, config):
        ...
        self.weights_file = self.model_path.joinpath("weights.pth")
    ...
    def write_logs(self, logs, epoch):
        ...
        if (epoch % self.freq_log) == 0:
            ...
            for key, item in logs.items():
                ...
                try:
                    self.log[key].append(np.float64(item))
                # create list otherwise
                except KeyError:
                    try:
                        self.log[key] = [np.float64(item)]
                    # if list is given
    ...
    def save_weights(self, neural_net):
        ...
        torch.save(neural_net.state_dict(), self.weights_file)
