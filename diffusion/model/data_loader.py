...
class DataLoader:
    ...
    def analytical_solution(self, X):
        ...
        u = torch.sin(torch.pi * x / self.L) * torch.exp(
            -torch.pi**2 * t / self.tau
        )
    ...
    def regular_grid(self, N=128):
        ...
        X = self.array2tensor(
            np.vstack((xx.flatten(), tt.flatten())).T, exp_dim=False
        )
        # solution
    ...
    def sample_IC(self, N=128):
        ...
        X_IC = self.array2tensor([[x, 0] for x in x_ticks], exp_dim=False)
        # IC temperature
        u_IC = self.array2tensor(
            [[np.sin(np.pi * x / self.L)] for x in x_ticks], exp_dim=False
        )
